<p align="center">
  <img src="./assets/readme/roadmap.svg" width="100%" alt="실습 여섯 단계 — 화면 켜기, 로트 물어보기, 대화 저장, PRD 직접 쓰기, 승인 처리, 점수 내기. 그리고 시작 전에 알아야 할 데이터 함정 세 가지 — 설비 이상과 수율에 상관이 없다는 것, AI 에이전트 다섯 개 중 두 개만 데이터에 나온다는 것, 인터록과 계측오차와 담당 에이전트가 하나의 신호라는 것">
</p>
...
맨 위 다이어그램의 여섯 단계를 따릅니다. 1~3은 재료가 이어지므로 순서를 지킵니다 —
대화를 해야 리포트가 생기고, 리포트가 있어야 PRD를 쓸 재료가 생깁니다. 5·6은 아무 때나
해도 됩니다.

다이어그램 아래쪽에 있는 **데이터 함정 셋**은 모르고 시작하면 없는 인과를 지어내게 되는
지점입니다. 판정과 수치 근거는 [decisions.md](decisions.md) D-004 ~ D-006에 있습니다.
...
- [decisions.md](decisions.md) — **되돌리기 어려운 선택과 그 근거.** 데이터셋 경로,
  가드레일 수정, 그리고 `dataset_2`에 심어진 **데이터 함정 셋에 대한 판정**(D-004 ~ D-006)이
  수치 근거와 함께 기록되어 있습니다. 특히 이 데이터에는 설비 이상과 수율 사이에 상관이
  없어(`+0.005`) 인과 분석의 정답이 "UNKNOWN"이라는 점 — 실습을 시작하기 전에 읽으면
  같은 분석을 다시 하지 않아도 됩니다.

...
1~3은 재료가 이어지므로 순서를 지킵니다 — 대화를 해야 리포트가 생기고, 리포트가 있어야
PRD를 쓸 재료가 생깁니다. 5·6은 아무 때나 해도 됩니다.

아래쪽 **데이터 함정 셋**은 모르고 시작하면 없는 인과를 지어내게 되는 지점입니다.
판정과 수치 근거는 [decisions.md](decisions.md) D-004 ~ D-006에 있습니다.

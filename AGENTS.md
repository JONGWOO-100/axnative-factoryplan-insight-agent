...
**작업을 시작하기 전에 [decisions.md](decisions.md)를 먼저 읽는다.** 이 파일이
"규칙"이라면 그쪽은 "이미 내린 판정과 그 근거"다. 특히 `dataset_2`에 심어진 데이터
함정 셋(D-004 ~ D-006)은 모르고 건드리면 없는 인과를 코드로 굳히거나, 기준선 데이터를
"고쳐서" 무엇을 기준으로 채점했는지를 지워버리게 된다. 되돌리기 어려운 선택을 새로
했다면 같은 형식(**결정 / 근거 / 대안과 기각 사유**)으로 그쪽에 추가한다.
...
- **기준선 데이터를 "고치지" 않는다.** `dataset_2`에는 실습용으로 심어진 함정이
  있고, 그중 하나는 설비 이상과 수율 사이에 상관이 없다는 것이다
  (`corr = +0.005`). 상관관계를 주입해 시나리오가 "돌아가게" 만들면 무엇을
  기준으로 채점했는지가 사라진다. 고치는 쪽은 데이터가 아니라 시스템의 해석과
  표현이다 — [decisions.md](decisions.md) D-004 ~ D-006 참조.
- **인과를 확정하지 않는다.** 통합 리포트의 `causal_verdict`는 항상 `UNKNOWN`이고
  사유를 함께 싣는다. `guardrails.ALLOWED_CAUSAL_VERDICTS`를 넓히는 것은 제대로 된
  인과 분석(비교군 설계, 시간 추세 통제)을 붙이는 변경의 명시적인 일부여야 한다.
  거기서 걸리는 것은 버그가 아니라 의도된 마찰이다 — D-007 참조.
- **같은 근거를 여러 번 세지 않는다.** 인터록 · VM 오차 · 담당 에이전트는 하나의
  물리 사건에서 파생된 셋이다. 나란히 적으면 독자가 근거 3개로 읽는다 — D-006 참조.
- **가드레일을 조용히 완화하지 않는다.** 실패하는 테스트를 통과시키려고
  `harness/guardrails.py`의 검증 조건을 느슨하게 만들 때는 왜 완화하는지
  커밋 메시지에 남긴다. (계산 오류를 고치는 것은 완화가 아니다 — D-003이 그 예다.)
- **테스트가 실제 산출물 디렉터리에 쓰게 두지 않는다.** `conftest.py`의
  `isolate_writable_dirs`가 `runs/` · `outputs/` · `approvals/`를 `tmp_path`로 돌린다.
  산출물 경로를 import 시점에 상수로 굳히는 모듈을 새로 만들면
  `_WRITABLE_PATH_BINDINGS`에 추가한다. 격리가 빠지면 픽스처 로트가 사람이 보는
  승인 큐에 쌓여 HITL 자체를 믿을 수 없게 된다.

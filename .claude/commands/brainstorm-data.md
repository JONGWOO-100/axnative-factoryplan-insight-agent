---
description: dataset_2를 MCP 툴로만 저비용 분석해 브레인스토밍 아이디어를 뽑는다 (원본 CSV/xlsx는 읽지 않음)
---
...
- 원본 파일 접근 금지: CSV/xlsx를 `Read`/`cat`으로 직접 열지 않는다.
</context_budget>
...
1. 인자가 없으면(`$ARGUMENTS`가 비어 있으면) graph_stats -> get_fdc_anomalies ->
   get_yield_defects -> get_dx_kpi_trend 순으로 기본 4회만 호출한다.
2. 인자로 주제(`fdc`, `yield`, `kpi`, `graph` 중 하나 이상)가 주어지면 그 주제에
   해당하는 툴만 호출하고 나머지는 건너뛴다.
